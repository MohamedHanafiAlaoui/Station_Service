...
public class BadgeServiceImpl implements BadgeService {
    ...
    @Override
    public String generateUniqueRFID() {
        String rfid;
        do {
            rfid = "RFID" + (int)(Math.random() * 900000 + 100000);
        } while (clientRepository.existsByBadgeRFID(rfid));
        return rfid;
    }
    @Override
    @Transactional
    public String assignNewBadge(Long clientId) {
        ...
        String newRfid = generateUniqueRFID();
        client.setBadgeRFID(newRfid);
        clientRepository.save(client);
        JournalAuditDto audit = new JournalAuditDto();
        audit.setTypeAction("ASSIGN_BADGE");
        audit.setDescription("Attribution d'un nouveau badge (RFID: " + newRfid + ") au client: " + client.getNom());
